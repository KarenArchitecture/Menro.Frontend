...
export default function AdsSettingsModal({ onClose }) {
  ...
  async function loadSettingsFromServer(typeKey) {
    ...
    try {
      const response = await adSettingsAxios.get("", {
        params: { placement },
      });
    ...
    <div className="adsettings-overlay" onClick={() => !saving && onClose?.()}>
      <div className="adsettings-modal" onClick={(e) => e.stopPropagation()}>
        <div className="adsettings-modal__header">
          <h3>
            <i className="fas fa-cog" /> تنظیمات تبلیغات
          </h3>
          <button
            className="btn-icon"
            onClick={onClose}
        ...
        <div className="adsettings-modal__body">
