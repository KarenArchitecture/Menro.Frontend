...
import {
    ...
    const headerTitle = isCategoryMode
        ? (categoryTitleFromState ? `${categoryTitleFromState}‌های پرطرفدار` : "محبوب‌ترین غذاها")
        : "محبوب‌ترین غذاها";

    const header = (
        <SectionHeader title={headerTitle} linkText="بازگشت" to="/" />
    );
    ...
    return (
        ...
        <section className="previous-orders">
            ...
            {items.map((item) => (
                <div key={item.id ?? item.foodId} className="food-card-wrap--search">
                <FoodCard item={item} />
                </div>
            ))}
            </div>
