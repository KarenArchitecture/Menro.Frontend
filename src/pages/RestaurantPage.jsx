...
import { getRestaurantPageFoodCategoriesBySlug } from "../api/foodCategories";
import SearchIcon from "../components/icons/SearchIcon";
import CartIcon from "../components/icons/CartIcon";
import ProfileIcon from "../components/icons/ProfileIcon";
import CheckoutBar from "../components/shop/CheckoutBar";
...
function RestaurantContent() {
  ...
  const { data: menuData = { restaurant: null, categories: [] }, isLoading: menuLoading } = useQuery({
    queryKey: ["restaurantMenu", slug],
    queryFn: () => getRestaurantPageFoodCategoriesBySlug(slug),
  });

  /* ---------- Categories  ---------- */
  const categoriesWithAll = useMemo(() => {
    const apiCats = menuData.categories.map((sec) => ({
      id: String(sec.id),
      name: sec.name,
      svgIcon: sec.svgIcon,
      foods: sec.foods, // attach foods for MenuList
    }));
    return [{ id: "all", name: "همه", svgIcon: ALL_CAT_SVG, foods: menuData.categories.flatMap(c => c.foods) }, ...apiCats];
  }, [menuData]);
